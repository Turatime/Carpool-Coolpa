...
def create_booking(booking_data: BookingCreate, db: Session = Depends(get_db)):
    ...
    if passenger.balance < total_price:
        raise HTTPException(status_code=400, detail=f"Insufficient balance. Required: {total_price}, Current: {passenger.balance}")
...
def confirm_booking(booking_id: int, db: Session = Depends(get_db)):
    ...
    if booking.status != "paid":
        raise HTTPException(status_code=400, detail="Booking must be paid before confirmation")
